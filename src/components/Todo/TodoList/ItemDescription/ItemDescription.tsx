...
const ItemDescription = () => {
  ...
  if (loading) {
    ...
    content = (
      <Fragment>
        <p className={classes.Name}><span style={{color: dark}}>{t('name')}: </span>{currentTodo.name}</p>
        <p className={classes.Date}><span style={{color: dark}}>{t('created_at')}: </span>{currentTodo.date}</p>
        <p className={classes.Status}><span style={{color: dark}}>{t('status')}: </span>{t(currentTodo.status)}</p>
      </Fragment>
    );
  }
